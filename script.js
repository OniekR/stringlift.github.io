...
console.log('lift script loaded');
...
function updateOuterCustomFromSelect() {
  ...
  if (isNaN(od_m)) { if (el) el.value = ""; return; }
...
function updateInnerCustomFromSelect() {
  ...
  if (isNaN(id_m) || id_m <= 0) { if (el) el.value = "0"; return; }
...
function calculateLift() { try {
  ...
  const innerCustomRaw = document.getElementById("innerD_custom").value.trim();
  ...
  if (innerCustomRaw !== "") {
    ...
    if (isNaN(parsed) || parsed < 0) {
      ...
      explainEl.textContent = "Custom inner diameter must be a non-negative number (supports fractions like '5 7/8').";
      return;
    }
  ...
  const odSelectLabel = document.getElementById("outerD").selectedOptions[0].text;
  const outerCustomRaw = (document.getElementById("outerD_custom") || {}).value || "";
  ...
  if ((outerCustomRaw || "").trim() !== "") {
    ...
    if (isNaN(parsedOuter) || parsedOuter <= 0) {
      ...
      if (resultValueEl) resultValueEl.textContent = "— tons";
      resultEl.textContent = "";
      explainEl.textContent = "Custom outer diameter must be a positive number (supports fractions).";
      return;
    }
  ...
  if (id_m < 0 || od_m <= 0 || p_pa < 0) {
    ...
    if (resultValueEl) resultValueEl.textContent = "— tons";
    resultEl.textContent = "";
    explainEl.textContent = "Diameters must be non-negative and pressure cannot be negative.";
    return;
  }
  ...
  if (od_m <= id_m) {
    ...
    if (resultValueEl) resultValueEl.textContent = "— tons";
    resultEl.textContent = "";
    explainEl.textContent = "Outer diameter must be larger than inner diameter.";
    return;
  }
  ...
  const od_in = od_m / 0.0254;
  explainEl.innerHTML = `ID: ${idLabel} (${formatNumber(id_m, 5)} m)<br>OD: ${odLabel} (actual ${formatNumber(od_in, 3)} in / ${formatNumber(od_m,5)} m)<br>${caption}: ${formatNumber(usedArea, 6)} m²<br>Pressure: ${formatNumber(p_pa, 3)} Pa<br>Lift: ${formatNumber(liftTon,1)} tons (${formatNumber(liftKg,1)} kgf / ${formatNumber(liftN,1)} N)<br>Calculation: F = p × A`; 
  } catch (err) {
...
if (clearBtn) {
  ...
  console.warn('Clear button #clearBtn not found; skipping clear handler attach');
} 
